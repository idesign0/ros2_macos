...
public:
  static void SetUpTestCase()
  {
    ...
    config_->blackboard-> template set<std::chrono::milliseconds>("server_timeout", 20ms);
    config_->blackboard-> template set<std::chrono::milliseconds>("bt_loop_duration", 10ms);
    config_->blackboard-> template set<std::chrono::milliseconds>("wait_for_service_timeout", 1000ms);
...
TEST_F(BTActionNodeTestFixture, test_server_timeout_failure)
{
  ...
  config_->blackboard-> template set<std::chrono::milliseconds>("server_timeout", 90ms);
  config_->blackboard-> template set<std::chrono::milliseconds>("bt_loop_duration", 10ms);
...
TEST_F(BTActionNodeTestFixture, test_server_cancel)
{
  ...
  // setting a server timeout smaller than the time the action server will take to accept the goal
  // to simulate a server timeout scenario
  config_->blackboard-> template set<std::chrono::milliseconds>("server_timeout", 100ms);
  config_->blackboard-> template set<std::chrono::milliseconds>("bt_loop_duration", 10ms);
